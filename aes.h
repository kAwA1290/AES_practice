...
using state_t = array<array<uint8_t, STATE_ROWS>, STATE_COLS>;
...
public:
	...
	state_t cipher(const state_t in, uint8_t Nr, const vector<uint32_t> w);
	void subBytes(state_t& state);
	void shiftRows(state_t& state);
	void mixColumns(state_t& state);
	void addRoundKey(uint8_t round, state_t& state, uint8_t w[Nb]);
